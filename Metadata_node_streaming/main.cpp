...
static int read_frame(int fd, const std::string& dev_name, struct buffer *buffers)
{
    ...
    unsigned int i;
    __u32 buf_type = (dev_name=="/dev/video0") ? V4L2_BUF_TYPE_VIDEO_CAPTURE : V4L2_BUF_TYPE_META_CAPTURE;
...
static void mainloop(void)
{
    ...
    //                    }
                        ...
                        {
                            //handle_message(fd);
                            std::string dev_name = (fd==fd_0) ? dev_name_0 : dev_name_1;
                            struct buffer * buf = (fd==fd_0) ? buffers_0 : buffers_1;
                ...
                std::cout << cnt << " descriptors were trigered" << std::endl;
                if (cnt!=2)
                    exit(1);
            }
...
//    }
    ...
    if (force_format && (V4L2_BUF_TYPE_META_CAPTURE==type)) {

        fmt.fmt.meta.dataformat = V4L2_META_FMT_D4XX; //V4L2_META_FMT_UVC; // V4L2_META_FMT_D4XX; Evgeni  - D4XX format requires kernel patch
    ...
    struct buffer **bufs = (type == V4L2_BUF_TYPE_META_CAPTURE) ? &buffers_1 : &buffers_0;
...
int main(int argc, char **argv)
{
    ...
    open_device(&fd_1,dev_name_1);
    init_device(fd_0,dev_name_0);
    init_device(fd_1,dev_name_1);
    start_capturing(fd_0,buffers_0,dev_name_0);
    start_capturing(fd_1,buffers_1,dev_name_1);
    mainloop();
    stop_capturing(fd_1,dev_name_1);
    stop_capturing(fd_0,dev_name_0);
    uninit_device(fd_1,dev_name_1,buffers_1);
    uninit_device(fd_0,dev_name_0,buffers_0);
    close_device(fd_1,dev_name_1);
