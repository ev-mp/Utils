...
def build_subtype_table(ir_filter: bool, ip65_filter: bool) -> bytes:
    filter_byte = 0x01 if ir_filter else 0x00
    ip65_byte = 0x01 if ip65_filter else 0x00
    sub_type_data = bytes([filter_byte, ip65_byte, 0x00, 0x00])
...
def update_device_subtype(device, command, irFilter=None, ip65_filter=None):
    ...
    try:
        ...
        subtype_table = build_subtype_table(irFilter,ip65_filter)
        print(type(subtype_table))
        # Convert to list[int]
        st1 = [int(b) for b in subtype_table]  # Ensure integers 0–255
        print(f"Subtype table as list: {st1}")
    ...
    if (len(response) == 4 and response[0]==udw_command):
        print(f" {GREEN}Device ID in flash was modified{RESET}, re-reading the device sub-type info for confirmation:")
        parse_current_type(device)
    else:
        print(f"Response (hex): {[hex(x) for x in response]}")
        print(f"{RED}updating Flash NVM has failed. Copy and share the above response for analysis{RESET}")

def parse_current_type(device):
    # Query sub-type
    hw_monitor = device.as_debug_protocol()
    response=[]
    ...
    if not response or len(response) == 0:
        ...
        if len(response) < 24:
            ...
            # Parse bytes 20 and 21 (0-based index)
            ir_cut_filter = response[20]
            ip65_filter = response[21]

            #print(f"Status: {GREEN}OK{RESET}, Error: {RED}FAILED{RESET}")
            ir_cut_status = "Present" if ir_cut_filter == 1 else "Off"
            ir_cut_color = GREEN if ir_cut_filter == 1 else RED
            ip65_status = "Present" if ip65_filter == 1 else "Off"
            ip65_color = GREEN if ip65_filter == 1 else RED
            print(f"IR Cut Filter present: {ir_cut_color}{ir_cut_status}{RESET}")
            print(f"IP65 protection: {ip65_color}{ip65_status}{RESET}")
...
def yes_no_prompt(prompt: str) -> bool:
    while True:
        ...
        if answer in ['y', 'n']:
            return answer == 'y'
        print("{YELLOW}Invalid input{RESET}. Please enter 'y' or 'n'.")
...
def main():
    ...
    for dev in target_devices:
        ...
        print(f"\nInspect the device appearance/labels for the presence of IR Filter and/or IP65 protection labels")
        ir_filter = yes_no_prompt("Is the device has IR Cut filter installed? [y/N]: ")
        ip65_filter = yes_no_prompt("Is the device IP65-protected [y/N]: ")
        ack= yes_no_prompt(f"The following data {BOLD}{YELLOW}will be stored in device's Flash NVM{RESET}:\n\t {BLUE}IR Filter = {ir_filter}, IP65: {ip65_filter}{RESET}, do you confirm [y/N]? ")
        if (ack):
            print(f"\nUpdating device type:")
            update_device_subtype(dev, "UDW", ir_filter,ip65_filter)
        else:
